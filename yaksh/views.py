    ...
    if request.method == 'POST':
        ...
        if current_question.type == 'mcq':
            ...
            try:
                ...
                msg = _("Please enter an Integer Value")
                return show_question(
            ...
            try:
                ...
                msg = _("Please enter a Float Value")
...
def course_detail(request, course_id):
    user = request.user
    if not is_moderator(user):
        raise Http404(_('You are not allowed to view this page'))

    course = Course.objects.prefetch_related(
        'students', 'requests', 'rejected'
    ).get(id=course_id)
    if not course.is_creator(user) and not course.is_teacher(user):
        raise Http404(_('This course does not belong to you'))
...
def monitor(request, quiz_id=None, course_id=None, attempt_number=1):
    ...
    if not papers.exists():
        messages.warning(request, _("No AnswerPapers found"))
    else:
    ...
    if request.method == 'POST':
        ...
        if 'add-fixed' in request.POST:
            ...
            if question_ids:
                ...
                messages.warning(request, _("Please select atleast one question"))
        ...
        if request.POST.get('upload') == 'upload':
            ...
            if form.is_valid():
                ...
                if file_extension == "zip":
                    ...
                    message = _("Please Upload a ZIP file or YAML file")
...
def upload_users(request, course_id):
    ...
    if request.method == 'POST':
        if 'csv_file' not in request.FILES:
            messages.warning(request, _("Please upload a CSV file."))
            return my_redirect(reverse('yaksh:course_detail',
                                       args=[course_id]))
        csv_file = request.FILES['csv_file']
        is_csv_file, dialect = is_csv(csv_file)
        if not is_csv_file:
            messages.warning(request, _("The file uploaded is not a CSV file."))
            return my_redirect(reverse('yaksh:course_detail',
                                       args=[course_id]))
...
def upload_marks(request, course_id, questionpaper_id):
    ...
    if request.method == 'POST':
        ...
        if is_celery_alive:
            ...
            msg = _("Unable to submit for marks update. Please check with admin")
            messages.warning(request, msg)
    return redirect('yaksh:monitor', quiz.id, course_id)
...
def _read_marks_csv(request, reader, course, question_paper, question_ids):
    messages.info(request, _('Marks Uploaded!'))
    for row in reader:
            ...
            if qid in questions:
                ...
                if not answer:
                    answer = Answer(question_id=qid, marks=0, correct=False,
                                    answer=_('Created During Marks Update!'),
                                    error=json.dumps([]))
                    answer.save()
